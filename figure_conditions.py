...
\end{figure}
    ...
    if __name__ == "__main__":
        ...
        for idx, (method, item) in enumerate(items.items()):
            ...
            \addplot [thick, color""" + str(idx) + r""", mark=""" + ("*" if idx % 2 else "x") +  r""", mark size=2, mark options={solid}]
            table {
    """
    ...
    """
            ...
            for snr_db in np.arange(-2, 21, 1):
                macro_f1 = item(10**(0.1*snr_db)) # convert db to original
                res += "{:.2f}".format(snr_db) + "\t" + "{:.3f}".format(macro_f1) + "\n"
